...
def analyze_warning(request: WarningAnalyzeRequest, deepseek: DeepSeekClient):
    ...
    PDF要求：当错误次数 >= 3 且后端传入了 submissions 数据，
    ...
    if error_count < 3:
        logger.info(
            "Student %s error count %d < 3, no intervention needed",
            request.student_id,
            error_count,
        )
        return WarningResult(
